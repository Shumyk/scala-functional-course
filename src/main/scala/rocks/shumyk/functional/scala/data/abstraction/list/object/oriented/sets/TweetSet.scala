...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val currentIncluded = if p.apply(elem) then acc.incl(elem) else acc
    val leftIncluded = left.filterAcc(p, currentIncluded)
    right.filterAcc(p, leftIncluded)
  ...
  def mostRetweeted: Tweet =
    ...
    foreach(t => if t.retweets > mostRetweeted.retweets then mostRetweeted = t)
    mostRetweeted

  def descendingByRetweet: TweetList =
    val retweeted: Tweet = mostRetweeted
    Cons(retweeted, remove(retweeted).descendingByRetweet)
...
object GoogleVsApple:
  ...
  def searchTweetsByKeywords(keywords: List[String]): TweetSet =
    TweetReader.allTweets.filter(tweet => keywords.exists(keyword => tweet.text.contains(keyword)))

  lazy val googleTweets: TweetSet = searchTweetsByKeywords(google)
  lazy val appleTweets: TweetSet = searchTweetsByKeywords(apple)
